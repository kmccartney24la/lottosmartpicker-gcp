...
async function writeJsonDebug(name, data) {
    try {
...
async function numbersFromAemModel(page, label) {
    ...
    const candidates = [];
    const pushAbs = (p) => candidates.push(p.startsWith("http") ? `${p}.model.json` : `${origin}${p}.model.json`);
    if (reqPath)
        pushAbs(reqPath);
    ...
    pushAbs(urlPath);
    if (urlPath.startsWith("/en-us/"))
        pushAbs("/content/portal/en" + urlPath.slice("/en-us".length));
    let json = null;
    let used;
...
async function numbersFromModals(page, label) {
    const tileSelectors = [
...
export async function fetchActiveEndedNumbers(context) {
    const grab = async (url, label) => withRetry(async () => {
        ...
        try {
            ...
            if (nums.length === 0)
                nums = await numbersFromModals(page, label);
