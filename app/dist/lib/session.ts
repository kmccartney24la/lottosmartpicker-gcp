...
// In a real application, you would use a more robust session store (e.g., Redis, database)
// For this example, we'll simulate a session store with a simple in-memory object.
...
const sessions = new Map<string, SessionData>(); // Map session ID to session data
...
export function createSession(request: NextRequest): string {
