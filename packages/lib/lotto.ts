...
export type UnderlyingKey =
  ...
  | 'ny_pick10'
  | 'ny_quick_draw'
  // Florida Cash Pop (file-backed, flexible CSVs; 1 value)
  | 'fl_cashpop_morning'
  | 'fl_cashpop_matinee'
  | 'fl_cashpop_afternoon'
  | 'fl_cashpop_evening'
...
// Use a concrete shape to avoid distributive/union inference surprises.
type PeriodMap = {
  all: UnderlyingKey[];
  // 2-per-day
...
export function primaryKeyFor(logical: LogicalGameKey, period: Period): UnderlyingKey {
  ...
  if (!m && (GAME_TO_API_PATH as any)[logical]) {
    return logical as unknown as UnderlyingKey;
  }
...
export class NonCanonicalGameError extends Error {
 ...
 type CacheEnvelope = {
   ...
   rawCsv?: string;         // canonical CSV text
   // Back-compat:
   rows?: LottoRow[];       // legacy parsed cache (read-only; no longer written)
...
export const GAME_TO_API_PATH = Object.freeze(<Record<GameKey, string>>{
...
function apiPathForUnderlying(u: UnderlyingKey): string {
...
export function computeNextRefreshISO(_game?: GameKey): string {
 ...
 function writeCache(game: GameKey, payload: { rawCsv: string; rows?: LottoRow[] }, eraStart: string) {
   if (!isBrowser()) return;
...
export async function fetchRowsWithCache(options: {
  ...
  if (
    ...
    (game as any) === 'ca_daily3' ||
    (game as any) === 'ca_daily4'
  ) {
  ...
  if (!effectiveLatestOnly) {
    ...
    if (csvText) writeCache(game, { rawCsv: csvText }, era.start);
  }
...
async function fetchLatestDate(game: GameKey): Promise<string | null> {
  ...
  // Try HEAD first — cheap, cacheable, returns validators.
  ...
  if (head.ok) {
    ...
    // Some backends expose the date via ETag versioning; in that case we
    // can still return a non-null token so callers can compare equality.
    const etag = head.headers.get('etag');
    if (etag) return etag; // equality token (not a date); used only for equality checks
  }
  // Fallback: tiny GET (still cacheable) if HEAD is not supported / lacks validators.
  const res = await fetch(url);
...
const isCanonicalUnderlying = (k: UnderlyingKey): k is GameKey =>
  Object.prototype.hasOwnProperty.call(GAME_TO_API_PATH, k as any);
/**
...
export function drawNightsLabel(game: GameKey, now = new Date()): string {
  // Twice-daily games: keep the explicit wording
  if (
    game === 'ny_take5' || game === 'ny_take5_midday' || game === 'ny_take5_evening' ||
    game === 'fl_fantasy5_midday' || game === 'fl_fantasy5_evening'
  ) {
  ...
  if (!info) return dayPart; // no time known → just days
...
/* ---------------- Timezone-aware draw helpers (generalized) ---------------- */

type TZ = 'America/New_York' | 'America/Los_Angeles';
...
function tzAbbrev(tz: TZ, d = new Date()): string {
  ...
  if (/ET/i.test(abbr)) return 'ET';
  if (/PT/i.test(abbr)) return 'PT';
  if (/GMT[+-]\d+/.test(abbr)) return tz === 'America/New_York' ? 'ET' : 'PT';
...
export function isInDrawWindowFor(game: GameKey, now = new Date()): boolean {
...
export function nextDrawLabelFor(game: GameKey, now = new Date()): string {
...
export const USE_WORKER =
  typeof window !== 'undefined' && (
    ...
    ((typeof process !== 'undefined') && (process as any).env?.NEXT_PUBLIC_USE_WORKER === '1')
  );
...
async function _bridge(): Promise<typeof import('./workers/workerBridge')> {
  return await import('./workers/workerBridge');
}

export async function parseCanonicalCsvAsync(csv: string, game: GameKey, signal?: AbortSignal) {
  if (!USE_WORKER) return parseCanonicalCsv(csv, game);
  const { runTask } = await _bridge();
  return runTask<{csv:string;game:GameKey}, ReturnType<typeof parseCanonicalCsv>>('parseCanonicalCsv', { csv, game }, signal);
}

export async function parseFlexibleCsvAsync(csv: string, signal?: AbortSignal) {
  if (!USE_WORKER) return parseFlexibleCsv(csv);
  const { runTask } = await _bridge();
  return runTask<{csv:string}, ReturnType<typeof parseFlexibleCsv>>('parseFlexibleCsv', { csv }, signal);
}
...
export async function computeDigitStatsAsync(
  rows: DigitRow[], k: 3|4, signal?: AbortSignal
) {
  if (!USE_WORKER) return computeDigitStats(rows, k);
  const { runTask } = await _bridge();
  return runTask<{rows:DigitRow[];k:3|4}, ReturnType<typeof computeDigitStats>>(
...
export type DigitRowEx = DigitRow & { fb?: number };
