...
export async function fetchScratchersWithCache(): Promise<ActiveGame[]> {
  try {
    ...
    const res = await fetch('/api/scratchers', { cache: 'no-store' as any });
    if (!res.ok) {
...
export function sorters(key: SortKey, latestScores?: ReturnType<typeof rankScratchers>) {
  switch (key) {
    ...
    case 'adjusted':
      return (a: any, b: any) =>
        (a.adjustedOdds ?? Number.POSITIVE_INFINITY) - (b.adjustedOdds ?? Number.POSITIVE_INFINITY);

    case 'odds':
      return (a: any, b: any) =>
        (a.overallOdds ?? Number.POSITIVE_INFINITY) - (b.overallOdds ?? Number.POSITIVE_INFINITY);

    case 'topPrizeValue':
      return (a: any, b: any) =>
        (b.topPrizeValue ?? -Infinity) - (a.topPrizeValue ?? -Infinity);

    case 'topPrizesRemain':
      return (a: any, b: any) =>
        (b.topPrizesRemaining ?? -Infinity) - (a.topPrizesRemaining ?? -Infinity);
