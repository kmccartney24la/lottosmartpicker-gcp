...
export function normalizeRowsLoose(rows: any[]): LottoRow[] {
  ...
  for (const r of rows) {
    ...
    const [n1, n2, n3, n4, n5] = mains as [number, number, number, number, number];
    ...
    const special =
      specialRaw !== undefined && specialRaw !== null && specialRaw !== ''
        ? Number(specialRaw)
        : undefined;
    if (special !== undefined && !Number.isFinite(special)) continue;
...
// ---- LRU memoization (keyed by game + era + lastRow + length) ----
  ...
  for (let i = rows.length - 1, idx = 0; i >= 0; i--, idx++) {
    const d = rows[i];
    if (!d) continue; // narrow away undefined from sparse or OOB indexes
...
export function recommendQuickDrawFromStats(
  ...
  if (stats) rec.alpha = clampAlphaGeneric(rec.alpha, stats.totalDraws, 80, 0.50, 0.70);
  return rec;
}
