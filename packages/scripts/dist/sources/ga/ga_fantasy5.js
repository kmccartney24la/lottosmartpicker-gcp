...
const DATE_RE = /\b(?:Sun|Mon|Tue|Wed|Thu|Fri|Sat|Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday),?\s+[A-Za-z]{3,9}\s+\d{1,2},?\s+\d{4}\b/;
// -------------------- Utilities --------------------
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const jitter = (base) => base + Math.floor(Math.random() * base);
const backoff = (attempt) => Math.min(F5_RETRY_MAX_MS, F5_RETRY_BASE_MS * 2 ** (attempt - 1)) +
...
function makeCsvLine(dateISO, nums) {
    if (!dateISO || !Array.isArray(nums) || nums.length !== 5)
        return null;
    if (nums.some((n) => !Number.isInteger(n) || n < 1 || n > 99))
        return null;
...
function parseArgs(argv = process.argv.slice(2)) {
    ...
    for (const a of argv) {
        const [k, v] = a.split("=");
        if (k === "--start-year")
            args.startYear = Number(v);
        else if (k === "--end-year")
            args.endYear = Number(v);
        else if (k === "--since")
            args.since = v;
        else if (k === "--ttl-hours")
            args.ttlHours = Number(v);
    }
    return args;
}
// -------------------- HTTP (native fetch) --------------------
    ...
    for (let attempt = 1; attempt <= F5_FETCH_RETRIES; attempt++) {
        try {
            ...
            if (status === 403 || status === 429)
                throw err;
            lastErr = err;
            if (attempt === F5_FETCH_RETRIES)
                break;
            const wait = backoff(attempt);
            console.warn(`[Fantasy5] fetch retry ${attempt} for ${url} in ${wait}ms: ${err?.message ?? String(err)}`);
            await sleep(wait);
        }
    }
...
async function fetchHtmlWithPlaywright(url) {
    if (!F5_ENABLE_PLAYWRIGHT)
        throw new Error("Playwright disabled by F5_ENABLE_PLAYWRIGHT=0");
...
// -------------------- Public API --------------------
export async function buildGeorgiaFantasy5Csv(outRelPath = "public/data/ga/fantasy5.csv") {
...
if (typeof process !== "undefined" && process.argv && process.argv[1]) {
    ...
    if (isDirect) {
        buildGeorgiaFantasy5Csv().catch((e) => {
            console.error(e);
            process.exitCode = 1;
        });
    }
