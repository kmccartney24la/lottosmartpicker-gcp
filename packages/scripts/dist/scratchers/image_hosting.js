...
// ---- one-time capability probes (top-level; used for logging & defaults)
function deriveBucketFromPublicBaseUrl(pub) {
    const u = (pub || "").trim();
    if (!u)
        return undefined;
    try {
...
function resolveBucketAndPublicBase() {
    ...
    const envPublicBase = (process.env.PUBLIC_BASE_URL ||
    ...
    //   - Else, empty ⇒ FS mode
    const publicBase = envPublicBase ||
...
const HAVE_GCS = !!_BUCKET && !!_PUB_BASE;
const HAVE_R2 = !!process.env.CLOUDFLARE_ACCOUNT_ID &&
...
export function setHostingOptions(opts) {
...
export async function loadManifestFromGCSIfAvailable() {
    ...
    try {
        ...
        const count = Object.keys(manifestCache).length;
...
function isLocalhostUrl(u) {
    try {
        return LOCALHOST_SET.has(new URL(u).hostname.toLowerCase());
    }
    catch {
        return false;
    }
 ...
 */
async function fetchBinaryWithHeaders(url, init) {
    ...
    try {
        const res = await fetchWithRetry(url, {
            ...(init || {}),
            headers: { ...DEFAULT_HEADERS, ...perHostHeaders, ...init?.headers },
        });
        const ct = (res.headers.get("content-type") || "").toLowerCase();
        if (!ct || ct.startsWith("text/html"))
            throw new Error(`unexpected content-type from undici: ${ct}`);
    ...
    try {
        ...
        if (!resp)
            throw new Error("playwright: no response");
        ...
        if (!buf)
            throw new Error("playwright: empty body");
...
class GCSProvider {
    ...
    constructor() {
        ...
        if (!bucket || !publicBase) {
            throw new Error("GCSProvider cannot resolve bucket/publicBase. " +
