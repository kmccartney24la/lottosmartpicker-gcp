...
export async function saveDebug(page, basename) {
...
export async function maybeStartTracing(context) {
...
export async function maybeStopTracing(context, outPath) {
    if (process.env.TRACE) {
        await ensureDir(DATA_DIR);
        try {
...
export function oddsFromText(text) {
    if (!text)
        return undefined;
...
/* ---------------- link harvesting ---------------- */
const harvestByPage = new WeakMap();
function normalizeGameUrl(u) {
    try {
        ...
        const m = p.match(/^\/en-us\/games\/scratchers\/([^\/?#]+)(?:\.html)?$/i) ||
...
export function attachNetworkHarvester(page) {
    const store = new Set();
    ...
    page.on("response", async (resp) => {
        try {
            ...
            for (const u of harvestUrlsFromText(body))
                store.add(u);
        }
        catch { /* ignore */ }
    });
}
export async function openAndReady(page, url, opts) {
...
export async function waitForNumericGameLinks(page, nameHint, minCount = 5, timeoutMs = 45_000) {
        ...
        const evalOnce = () => page.evaluate(() => {
            ...
            const allRoots = (root) => {
                ...
                const push = (el) => {
                    ...
                    if (sr)
                        acc.push(...Array.from(sr.querySelectorAll("*")));
                };
                const scope = root.querySelectorAll ? root.querySelectorAll("*") : [];
                // Ensure Array.from produces Element[] instead of unknown[]
                const elements = Array.from(scope);
                elements.forEach((el) => push(el));
                return acc;
            };
                ...
                for (const key of ["data-init", "data-items", "data-teaser", "data-props", "data-options"]) {
                    ...
                    try {
                        ...
                        for (const s of Array.from(bag)) {
                            if (/\/games\/scratchers\/[a-z0-9-]+(?:\.html)?/i.test(s))
                                out.add(abs(s));
                        }
                    }
            ...
            document.querySelectorAll('a[href*="/games/scratchers/"]').forEach((a) => {
                const href = a.getAttribute("href") || "";
                if (/\/games\/scratchers\/[a-z0-9-]+(?:\.html)?/i.test(href))
                    out.add(abs(href));
            });
            ...
            document.querySelectorAll('[class*="tile"],[class*="game"] a[href*="/games/scratchers/"]').forEach((a) => {
                const href = a.getAttribute("href");
                if (href)
                    out.add(abs(href));
            });
            ...
            scanEls.forEach((el) => {
                for (const n of attrs) {
                    ...
                    if (!v)
                        continue;
                    if (/\/games\/scratchers\/[a-z0-9-]+(?:\.html)?/i.test(v))
                        out.add(abs(v));
                }
            });
            // inline onclick(...)
            ...
            document.querySelectorAll('[data-json],[data-config],[data-model]').forEach((el) => {
                for (const n of ["data-json", "data-config", "data-model"]) {
                    const blob = el.getAttribute(n);
                    if (blob && /\/games\/scratchers\//i.test(blob)) {
                        const re = /(["'])([^"']*\/games\/scratchers\/[a-z0-9-]+(?:\.html)?)\1/gi;
                        let m;
    ...
    async function collectFromNetwork() {
        const harvested = harvestByPage.get(page) ?? new Set();
        return Array.from(harvested);
    }
    async function collectFromHtmlOuter() {
        try {
            const html = await page.content();
    ...
    async function collectFromWindow(page) {
        const strings = await page.evaluate(() => {
            ...
            try {
                ...
                for (const k of Object.keys(w)) {
                    ...
                    if (lk.includes("game") || lk.includes("tile") || lk.includes("scratch"))
                        push(w[k]);
                }
            }
            catch { }
    ...
    try {
        await fs.writeFile(path.join(DATA_DIR, `_debug_${nameHint}.links.json`), JSON.stringify({ count: results.length, links: results }, null, 2), "utf8");
    }
    catch { }
    if (results.length === 0) {
        await saveDebug(page, `_debug_links_${nameHint}_fail`);
        throw new Error(`waitForNumericGameLinks: timed out without reaching minCount=${minCount}; gathered=${results.length}`);
    }
    return results;
}
