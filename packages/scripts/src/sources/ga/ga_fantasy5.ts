...
// Playwright is imported dynamically at runtime; import its types only:
...
const LNN_YEAR = (y: number): string =>
  `https://www.lottonumbers.com/georgia-fantasy-5/numbers/${y}`;
...
const backoff = (attempt: number) =>
  Math.min(F5_RETRY_MAX_MS, F5_RETRY_BASE_MS * 2 ** (attempt - 1)) +
  Math.floor(Math.random() * 250);

function resolveOutPath(outRelPath: string): string {
...
function toISOFromDateLike(s: string | number | Date): string | null {
  const d = new Date(s);
...
function makeCsvLine(dateISO: string | null, nums: number[] | unknown): string | null {
...
// -------------------- HTTP (native fetch) --------------------
async function fetchTextOnce(url: string, extraHeaders: Record<string, string> = {}) {
...
async function fetchTextWithRetries(url: string, extraHeaders: Record<string, string> = {}) {
  let lastErr: unknown;
  for (let attempt = 1; attempt <= F5_FETCH_RETRIES; attempt++) {
    try {
      ...
      console.warn(
        `[Fantasy5] fetch retry ${attempt} for ${url} in ${wait}ms: ${err?.message ?? String(err)}`
      );
      await sleep(wait);
    }
  }
...
async function fetchHtmlOrBrowser(url: string): Promise<string> {
  try {
...
function lnParseYearCheerio($: CheerioAPI): string[] {
  const lines: string[] = [];

  $("table tr").each((_: number, tr: CheerioNode) => {
    const $tr = $(tr);
    ...
    let nums: number[] = $tr
      ...
      .toArray()
      .map((el: CheerioNode) => Number($(el).text().trim()))
      .filter((n) => Number.isInteger(n) && n >= 1 && n <= 99);

    if (nums.length < 5) {
      const ints: number[] = ($tr.text().match(/\b\d+\b/g) || [])
        .map(Number)
        .filter((n) => Number.isInteger(n) && n >= 1 && n <= 99);
...
async function fetchYearLinesLNN(year: number, ttlMs: number): Promise<string[]> {
  const url = LNN_YEAR(year);
  const html = await fetchCached(url, (u: string) => fetchHtmlOrBrowser(u), ttlMs);
  // Lazy-load cheerio at the point of use to avoid load-time resolution
...
// -------------------- Public API --------------------
export async function buildGeorgiaFantasy5Csv(
  outRelPath: string = "public/data/ga/fantasy5.csv"
): Promise<void> {
  const outPath = resolveOutPath(outRelPath);

  // CLI config
  const args = parseArgs();
  const startYear: number = Number.isInteger(args.startYear) ? (args.startYear as number) : FIRST_YEAR;
  const endYear: number = Number.isInteger(args.endYear) ? (args.endYear as number) : THIS_YEAR;
  const since: string = args.since ?? DEFAULT_SINCE;
  const ttlHours: number = Number.isFinite(args.ttlHours) ? (args.ttlHours as number) : 12;
  const ttlMs: number = ttlHours > 0 ? ttlHours * 3600 * 1000 : 0;
  ...
  try {
    for (let y = startYear; y <= endYear; y++) {
      try {
        ...
        console.warn(`[Fantasy5] ${y} failed: ${e?.message ?? String(e)}`);
      }
...
if (typeof process !== "undefined" && process.argv && process.argv[1]) {
  ...
  if (isDirect) {
    buildGeorgiaFantasy5Csv().catch((e: any) => {
      console.error(e);
      process.exitCode = 1;
    });
  }
