...
export type TopPrizeRow = {
  ...
  originalTopPrizes?: number;   // TOTAL
  topPrizesRemaining?: number;  // TOTAL - CLAIMED
  lastUpdated?: string;
};

const toMoney = (s?: string): number | undefined => {
...
export async function fetchTopPrizes(
  context: BrowserContext
): Promise<Map<number, TopPrizeRow>> {
  ...
  try {
    ...
    const { rows, lastUpdated } = await page.evaluate(() => {
      ...
      const readFromText = () => {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (/game\s*number/i.test(lines[i]) && /top\s*prize/i.test(lines[i])) { start = i; break; }
        }
        if (start < 0) return { rows: [] as any[], lastUpdated: undefined as string | undefined };

        const out: any[] = [];
        for (let i = start + 1; i < lines.length; i++) {
          const L = lines[i];
          if (/^\*/.test(L)) break; // footnotes reached
          const cells = L.split(/\t+/);
          if (cells.length < 6) continue;
          const [cGame, cName, cPrice, cTop, cClaim, cTotal] = cells.map((x: string) => x.trim());
          if (!/^\d{3,5}$/.test(cGame)) continue;
          out.push({
    ...
    const map = new Map<number, TopPrizeRow>();
    for (const r of rows) {
      ...
      if (!prev) {
        ...
        if (nextVal > prevVal) map.set(r.gameNumber, next);
      }
    }
