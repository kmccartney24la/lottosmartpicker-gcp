...
const nextConfig = {
  ...
  async headers() {
   ...
   webpack(config, { dev }) {
    // Avoid eval-based source maps in dev so CSP can omit 'unsafe-eval'
    if (dev) {
      // Good balance of speed + debuggability without using eval()
      config.devtool = 'cheap-module-source-map';
      // If you still see any eval warning on your platform, you can fallback to:
