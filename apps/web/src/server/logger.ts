// apps/web/app/src/server/logger.ts
...
async function sendToCloudLogging(logEntry: any): Promise<void> {
  try {
    ...
    try {
      // De-literalize so bundler won't try to resolve at build time
      const modName = '@google-cloud/logging';
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const cloudLogging: any = await (Function('m', 'return import(m)') as (m: string) => Promise<any>)(modName);
