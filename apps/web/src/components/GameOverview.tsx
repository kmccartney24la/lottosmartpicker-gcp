...
function GameOverviewInner({ game, logical, period = 'both' }: Props) {
  ...
  const [a, setA] = useState<ReturnType<typeof import('@lsp/lib').analyzeGame> | null>(null);
  ...
  return (
    <section className="card game-overview" role="note" aria-label="Game overview">
      ...
      <ul className="game-overview-list">
        ...
        {!busy && !err && digitStats && (
          <>
            <li><strong>Jackpot odds:</strong> 1 in {
              ((logical ? jackpotOddsForLogical(logical) : null) ?? jackpotOdds(game)).toLocaleString()
            }</li>
          </>
        )}
        ...
        {!busy && !err && pick10Stats && (
          <>
            <li><strong>Jackpot odds:</strong> 1 in {
              (jackpotOddsForLogical('ny_pick10') ?? jackpotOdds(game)).toLocaleString()
            }</li>
          </>
        )}
