...
const ScratchersTableInner = forwardRef<HTMLDivElement, ScratchersTableProps>(function ScratchersTable(
  ...
  /** The dedicated scrollport wrapper for desktop/tablet rows (Option B) */
  ...
  const updateBottomShadow = useCallback(() => {
    const scrollEl = scrollBoxRef.current ?? localRef.current;
  ...
  return (
    ...
    >
      ...
      {!hasRows ? null : (
      <div
        className="desktop-only tbody-scroll"
        ref={scrollBoxRef}
        role="region"
        aria-label="Scratchers rows"
      >
        <table className="compact w-full" role="table" aria-label="GA Scratchers comparison">
          <thead>
            <tr>
              {showThumbs && (
              ...
              <th className={`col-left  mono text-right ${showThumbs ? 'w-[14%]' : 'w-[12%]'}`} aria-sort="none">
                {canToggleLeft ? (
                  <button
                    type="button"
