...
export default function FiltersPanel(props: FiltersPanelProps) {
  ...
  // Unified swipe-to-close (matches PastDrawsSidebar)
  const { ref: drawerRef, dragging, touchHandlers } = useDrawerSwipe({
    open,
  ...
  const inner = (
    <>
      ...
      <div className="filters-header">
        {/* Title block: use the class CSS expects for grid-area: stack */}
           ...
           {/* Drawer close button must be a direct grid child to occupy grid-area: close */}
           {drawerMode && onClose && (
          <button
            type="button"
          ...
          {/* Inline-like controls: Reset (text) — remains its own grid area (reset) */}
  ...
  return createPortal(
    <>
      ...
      <aside
        ref={drawerRef}
        className={[drawerClasses, dragging ? 'dragging' : ''].join(' ')}
        id={drawerId}
        role="dialog"
        aria-modal="true"
