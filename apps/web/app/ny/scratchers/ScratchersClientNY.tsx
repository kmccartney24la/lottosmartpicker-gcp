// app/ny/scratchers/ScratchersClientNY.tsx
'use client';
/* eslint-disable no-console */
import { useMemo, useState, useEffect, useRef } from 'react';
import dynamic from 'next/dynamic';
import FiltersPanel, { type Filters } from 'apps/web/src/components/scratchers/FiltersPanel';
import ScratchersTable from 'apps/web/src/components/scratchers/ScratchersTable';
import { useScratchersIndex } from 'apps/web/src/components/scratchers/useScratchersIndex';
import { comparators } from 'apps/web/src/components/scratchers/sort';
import type { ActiveGame, SortKey } from 'apps/web/src/components/scratchers/types';
import type { DisplayMode } from 'apps/web/src/components/scratchers/DisplayModeSwitcher';
import { useIsMobile, useDrawerMode } from '@lsp/lib/react/breakpoints';

const LS_SHOW_ODDS = 'lsp.showOddsByDefault';
const AdsLot = dynamic(() => import('apps/web/src/components/ads/AdsLot'), { ssr: false });

export default function ScratchersClientNY() {
  const tableScrollRef = useRef<HTMLDivElement | null>(null);
  const shellRef = useRef<HTMLDivElement | null>(null);

  // ⬇️ Only change vs GA: pass jurisdiction = 'ny'
  const { games, updatedAt, loading } = useScratchersIndex({ jurisdiction: 'ny' });
  ...
  const sorted = useMemo(() => {
    ...
    out.sort((a, b) => {
      ...
      switch (key) {
        case 'best': {
          // Delegate to shared comparator (which already handles totals if present)
        ...
        case 'startDate': {
          // NY fallback:
          // • Prefer newer launch dates (desc)
          // • If same date OR missing dates, use gameNumber DESC (higher = newer)
          const ts = (s?: string) => {
            const t = Date.parse(s ?? '');
            return Number.isFinite(t) ? t : NaN;
          };
          const A = ts(a.startDate), B = ts(b.startDate);
          const aHas = Number.isFinite(A), bHas = Number.isFinite(B);
          if (aHas && bHas) {
            if (A !== B) { result = B - A; break; }           // newer date first
            result = b.gameNumber - a.gameNumber; break;      // same date → higher game #
          }
          if (aHas && !bHas) { result = -1; break; }          // dated before undated
          if (!aHas && bHas) { result =  1; break; }
          result = b.gameNumber - a.gameNumber;               // neither dated → higher game #
          break;
        }
