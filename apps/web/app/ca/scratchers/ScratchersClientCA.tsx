...
export default function ScratchersClientCA() {
  const tableScrollRef = useRef<HTMLDivElement | null>(null);
  const shellRef = useRef<HTMLDivElement | null>(null);

  // ⬇️ California jurisdiction
  const { games, updatedAt, loading } = useScratchersIndex({ jurisdiction: 'ca' });
  ...
  useEffect(() => {
    const v = localStorage.getItem('lsp.globalImageKind');
    if (v === 'odds') setGlobalImageKind('odds');
  }, []);
  ...
  const sorted = useMemo(() => {
    ...
    out.sort((a, b) => {
      ...
      switch (key) {
        ...
        case 'startDate': {
          // Prefer newer launch dates (desc), fallback to higher game number
          const ts = (s?: string) => {
          ...
          if (aHas && bHas) { if (A !== B) { result = B - A; break; } result = b.gameNumber - a.gameNumber; break; }
          if (aHas && !bHas) { result = -1; break; }
          if (!aHas && bHas) { result =  1; break; }
          result = b.gameNumber - a.gameNumber;
          break;
        }
        case 'name': default:
