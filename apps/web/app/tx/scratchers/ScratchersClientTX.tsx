...
export default function ScratchersClientTX() {
  const tableScrollRef = useRef<HTMLDivElement | null>(null);
  const shellRef = useRef<HTMLDivElement | null>(null);

  // ⬇️ Only change vs NY: pass jurisdiction = 'tx'
  const { games, updatedAt, loading } = useScratchersIndex({ jurisdiction: 'tx' });
  ...
  const sorted = useMemo(() => {
    ...
    out.sort((a, b) => {
      ...
      switch (key) {
        ...
        case 'startDate': {
          // TX uses launch dates; same fallback logic as NY
          const ts = (s?: string) => {
